...
# Masukkan semua 739 di sini

# 1. Download data sekaligus (Batch download lebih cepat daripada satu-satu)
print("Sedang menarik data dari Yahoo Finance... Mohon tunggu.")
data = yf.download(tickers, period="2y", interval="1d", group_by='ticker')

# 2. Inisialisasi Matriks untuk Screening
# Kita buat DataFrame kosong untuk menyimpan 'Snapshot' terakhir
matrix = pd.DataFrame(index=tickers)

# 3. Loop untuk mengisi Matriks dengan Last Price & Last Volume
for ticker in tickers:
    try:
        ...
        # Isi matriks
        matrix.loc[ticker, 'Last_Price'] = last_row['Close']
        matrix.loc[ticker, 'Last_Volume'] = last_row['Volume']
        
        # Tambahan: Ambil SMA Volume 20 buat bahan rumus nanti
        matrix.loc[ticker, 'Avg_Vol_20'] = data[ticker]['Volume'].tail(20).mean()
        
    except Exception as e:
        print(f"Gagal narik data {ticker}: {e}")

print("\n--- MATRIKS HASIL SCREENING ---")
print(matrix) 