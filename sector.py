...
print(f"Sedang menarik data untuk {len(tickers)} ticker...")

# Menggunakan ThreadPoolExecutor agar lebih cepat (Paralel)
with ThreadPoolExecutor(max_workers=10) as executor:
    results = list(executor.map(get_sector, tickers))

# Hitung jumlah per sektor
sector_counts = Counter(results)
total = len(results)
...
print("\n--- Universe Sector Summary ---")
print(df_summary.to_string(index=False))

# Opsional: Simpan ke CSV untuk dashboard "Quant Radar" kamu
